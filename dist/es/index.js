...
import { openBlock, createElementBlock, createElementVNode, defineComponent, computed, createBlock, resolveDynamicComponent, reactive, ref, watch, onUnmounted, provide, inject, onMounted, watchEffect, toRefs, normalizeClass, normalizeStyle, renderSlot, unref, withKeys, createVNode, createCommentVNode, withDirectives, createTextVNode, toDisplayString, resolveDirective, Fragment, renderList, mergeProps, toHandlers, resolveComponent, nextTick, Transition, withCtx, withModifiers, normalizeProps, guardReactiveProps, toRef, isRef, h } from "vue";
...
const IconChevronRight = /* @__PURE__ */ _export_sfc(_sfc_main$g, [["render", _sfc_render$8]]);
const _sfc_main$f = {};
const _hoisted_1$b = {
  "stroke-linecap": "round",
  "stroke-linejoin": "round",
...
const IconChevronLeft = /* @__PURE__ */ _export_sfc(_sfc_main$f, [["render", _sfc_render$7]]);
const _sfc_main$e = {};
const _hoisted_1$a = {
  "stroke-linecap": "round",
  "stroke-linejoin": "round",
...
const IconChevronDown = /* @__PURE__ */ _export_sfc(_sfc_main$e, [["render", _sfc_render$6]]);
const _sfc_main$d = {};
const _hoisted_1$9 = {
  fill: "none",
  "stroke-linecap": "round",
...
const IconClock = /* @__PURE__ */ _export_sfc(_sfc_main$d, [["render", _sfc_render$5]]);
...
const hasAny = (obj, props) => _some(props, (p) => _has());
...
function useBase() {
  const context = inject(contextKey$3);
  if (context)
    return context;
  throw new Error(
...
function createCalendar(props, { emit, slots }) {
  ...
  const firstPage = computed(() => head$1(state2.pages) ?? null);
  const lastPage = computed(() => last$1(state2.pages) ?? null);
  ...
  const isMonthly = computed(() => state2.view === "monthly");
  const isWeekly = computed(() => state2.view === "weekly");
  const isDaily = computed(() => state2.view === "daily");
  const onTransitionBeforeEnter = () => {
    state2.inTransition = true;
    emit("transition-start");
  };
  const onTransitionAfterEnter = () => {
    state2.inTransition = false;
    emit("transition-end");
  ...
  const addPages$1 = (address, count2, view = state2.view) => {
    return addPages(address, count2, view, locale.value);
  };
  const getDateAddress = (date) => {
    return getPageAddressForDate(date, state2.view, locale.value);
  };
  const refreshDisabledDay = (day) => {
  ...
  const refreshFocusableDay = (day) => {
    day.isFocusable = day.inMonth && day.day === state2.focusableDay;
  };
  ...
  const allDays = computed(
    () => state2.pages.reduce((result, page) => {
  ...
  const attributes = computed(() => {
    ...
    (props.attributes || []).forEach((attr, i) => {
      if (!attr || !attr.dates)
        return;
      const key = has$1(attr, "key") ? attr.key : i;
  ...
  const getTargetPageRange = (page, opts = {}) => {
    const { view = state2.view, position = 1, force } = opts;
  ...
  const refreshPages = (opts = {}) => {
    return new Promise((resolve, reject) => {
      ...
      state2.transitionName = getPageTransition(
        state2.pages[0],
        pages[0],
        transition
      );
      state2.pages = pages;
      if (state2.transitionName && state2.transitionName !== "none") {
        transitionPromise = {
          resolve,
          reject
        };
  ...
  const move = async (target, opts = {}) => {
    ...
    if (opts.fromPage && !pageIsEqualToPage(opts.fromPage, firstPage.value)) {
      if (state2.navPopoverRef) {
        state2.navPopoverRef.hide({ hideDelay: 0 });
      }
      if (opts.view) {
        skipWatcher("view", 10);
        state2.view = opts.view;
      }
      await refreshPages({
        ...opts,
        page: opts.fromPage,
  ...
  const onKeydown = (event) => {
    const day = state2.lastFocusedDay;
    if (day != null) {
  ...
  onMounted(() => {
    if (!props.disablePageSwipe && state2.containerRef) {
  ...
  watch(
    ...
    () => {
      ...
      emit("update:view", state2.view);
    }
  );
  watch(
    () => state2.focusableDay,
    () => {
      forDays(state2.pages, (day) => refreshFocusableDay(day));
    }
  );
  watchEffect(() => {
    emit("update:pages", state2.pages);
    forDays(state2.pages, (day) => {
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", {
        ...
        show.value.title ? withDirectives((openBlock(), createElementBlock("div", _hoisted_1$8, [
...
function _sfc_render$4(_ctx, _cache, $props, $setup, $data, $options) {
  ...
  return openBlock(), createElementBlock("div", {
    ...
    renderSlot(_ctx.$slots, "day-content", {
      day: _ctx.day,
      attributes: _ctx.attributes,
      attributeCells: _ctx.attributeCells,
  ...
  return openBlock(), createElementBlock("div", {
    ...
    createElementVNode("div", {
      ...
      createElementVNode("div", _hoisted_1$6, [
        (openBlock(true), createElementBlock(Fragment, null, renderList($props.page.weekdayLabels, (wl, i) => {
          return openBlock(), createElementBlock("div", {
            key: i,
            class: "vc-weekday"
          }, toDisplayString(wl), 1);
        }), 128))
      ]),
...
function createCalendarNav(props, { emit }) {
  ...
  const title2 = computed(() => {
    return monthMode.value ? yearIndex.value + (props.buddhist ? 543 : 0) : `${(firstYear.value ?? 0) + (props.buddhist ? 543 : 0)} - ${(lastYear.value ?? 0) + (props.buddhist ? 543 : 0)}`;
  });
...
function createDatePicker(props, ctx) {
  ...
  function sortRange(range, priority = "none") {
  ...
  async function moveToValue(target, opts = {}) {
    if (calendarRef.value == null || target === "none")
      return false;
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", {
        class: normalizeClass(["vc-time-picker", [{ "vc-invalid": !unref(isValid), "vc-bordered": unref(showBorder) }]])
      }, [
